...
def show_start():
    ...
    x, y = list(map(int, root.geometry().split('+')[1:]))
    if run:
        width, height = list(map(int, root.geometry().split('+')[0].split('x')))
        x += (width - MNU.SCR_W) / 2
...
def verify_HST():
    ...
    try:
        ...
        for key in HST:
            ...
            if isinstance(HST[key], map):
                HST[key] = list(HST[key])
            assert isinstance(HST[key], list)
            for name in HST[key]:
                assert isinstance(name, str)
                names += 1
                assert names <= MNU.HST_H
        # I have no idea what is causing problems in the next line.
        # assert names == MNU.HST_H, repr(names) + ' != ' + repr(MNU.HST_H)
    except:
        root.withdraw()
